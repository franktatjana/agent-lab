    ...
    <section ref={ref} className="px-6 py-16 md:py-20">
      <div className={`max-w-5xl mx-auto ${isVisible ? "scene-fade-up" : "scene-hidden"}`}>
        ...
        <div className={`grid ${section.image ? "md:grid-cols-[1fr_380px]" : hasMetrics ? "md:grid-cols-[1fr_240px]" : ""} gap-6 items-start`}>
          <div className={`grid ${hasMetrics && !section.image ? "md:grid-cols-[1fr_240px]" : ""} gap-8`}>
            {/* Cards grid */}
            {section.cards && (
              <div className="grid grid-cols-2 gap-3">
                {section.cards.map((card, i) => (
                  <div
                    key={card.label}
                    className={`bg-white border border-stone-200 rounded-xl ${section.image ? "p-3" : "p-4"} ${isVisible ? "scene-scale-in" : "scene-hidden"}`}
                  ...
                  >
                    ...
                    <p className={`text-stone-800 ${section.image ? "text-sm" : "text-base"} leading-relaxed`}>{formatInlineText(card.detail)}</p>
                  </div>
                ))}
              </div>
            )}

            {/* Metrics sidebar (only when no image) */}
            {hasMetrics && !section.image && (
